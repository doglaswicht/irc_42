    ...
    Server &server)
{
    ...
    Client *client = db.get_co_client(fds[index].fd);
    ...
    db.move_co_client_to_deco(fds[index].fd);
    close(fds[index].fd);
    ...
    Server &server)
{
    ...
    {
        Client *client = db.get_co_client(fds[i].fd);
    ...
    while (server_is_running())
    {
        ...
        for (size_t i = 0; i < fds.size(); ++i)
        {
            ...
            if (fds[i].fd != fd_server && (fds[i].revents & POLLOUT))
            {
                ...
                Client *client = server.get_db().get_co_client(fds[i].fd);
            ...
            if (handle_client_data(fds[i].fd, server) != 0)
            {
